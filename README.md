...
│   │   │   │           └── page.tsx # Edit job
│   │   │   ├── applications/
│   │   │   │   ├── page.tsx         # My applications (testers)
│   │   │   │   └── [id]/
...
# 🚀 Payment Integration & Verification System - Setup Guide
...
```
1. Create Job → Fill form
2. Click "Create Job" → Job saved as DRAFT
3. Click "Publish Job" → Redirected to payment page
4. Enter payment details → Stripe payment form
5. Pay ${total_cost} → Payment processed
6. Job status → ACTIVE
7. Job visible to testers → Applications start coming
```

### Tester Flow with Verification:

```
1. Browse Jobs → See active jobs
2. Apply to job → Application submitted (PENDING)
3. Developer approves → Status: APPROVED
4. Opt-in to Google Play → Click link, join test
5. Upload screenshot → Verification image
6. Status → OPTED_IN
7. Developer verifies → Status: TESTING
8. Test for 14 days → Countdown active
9. Testing completes → Status: COMPLETED
10. Get paid → Automatic transfer to bank
```
...
✅ Escrow system (funds held)  
✅ Tester Connect onboarding  
✅ Automatic transfers after 14 days  
✅ Webhook handling  
✅ Payment status tracking  

### Verification System:
✅ Image upload with UploadThing  
✅ Screenshot preview for developers  
...
Option 3: Landing Page Update
   ...
   * Add "Get Started" CTA
   ...
   This is **very solid**. You’ve basically written a real internal **engineering runbook**. 👍
I’ll do three things for you, clearly and honestly:

1. ✅ **Confirm what’s correct**
2. ⚠️ **Point out what needs adjustment / is risky**
3. 🧭 **Give you a cleaned “final recommendation” version** so you don’t get blocked later by Stripe or Google Play

No fluff.

---

## 1️⃣ What you wrote that is ✅ correct
...
* Clear developer → tester → payout lifecycle
...
## 2️⃣ Important corrections & clarifications (very important)

### ⚠️ 1. Stripe does NOT call it “escrow”

Stripe is strict about wording.

You **must avoid** using the word **“escrow”** in:
...
> “Funds are temporarily held by the platform and released after service completion.”

Your implementation is fine — just the wording matters.

---

### ⚠️ 2. Automatic payouts after 14 days need a trigger
...
### ⚠️ 3. UploadThing images must be validated

Right now, “any image” is okay for testing, but for production you should:
...
### ⚠️ 4. Stripe Connect onboarding must be **before payout**
...
Stripe requires this. You’re mostly doing it already — just enforce it server-side too.

---

### ⚠️ 5. Webhook idempotency is critical
...
## 3️⃣ What I recommend changing in your guide (clean version)
...
## 5️⃣ What I strongly suggest next (pick one)

If you want, I can now help you with:

1️⃣ **Stripe payout cron job (code)**
2️⃣ **Webhook idempotency hardening**
3️⃣ **Google Play policy-safe wording for testers**
4️⃣ **Refund / cancellation logic**
5️⃣ **Admin “release payment” override**
